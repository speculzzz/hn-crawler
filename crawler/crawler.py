...
class HNCrawler:
    ...
    async def show_recent_news(self, limit: int = 5):
        """Выводит последние сохранённые новости с форматированием"""
        try:
            ...
            async for news_item in self.storage.get_recent_news(limit):
                print(str(news_item))
                # print(f"[{news_item.id}] {news_item.title}")
                # print(f"Score: {news_item.score} | Author: {news_item.author}")
                # print(f"URL: {news_item.url}")
                # print(f"Comments: {news_item.comments_url}")
                print("-" * 50)
